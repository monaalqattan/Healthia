...
function Toggle({ enabled, onChange }: { enabled: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={`relative w-10 h-6 rounded-full transition-all duration-300 cursor-pointer ${enabled ? "bg-green-600" : "bg-gray-300"}`}
    >
      <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${enabled ? "left-5" : "left-1"}`} />
    </button>
  )
}
...
const defaultSessions: Session[] = [
  ...
  { label: "Evening Session",   from: "17:00", to: "19:00", enabled: false },
]

// بنحول 24h لـ 12h للعرض
...
export default function ManageAvailability() {
  ...
  // عشان نعرف لو في تعديلات غير محفوظة
  ...
  const handleSave = () => {
    ...
    console.log("Saved availability:", { day: activeDay, sessions })
    setSaved(true)
    // الرسالة بتختفي بعد 3 ثواني
    ...
    <div className="bg-white rounded-2xl p-4 md:p-6 shadow-sm w-full">
      ...
      <div className="flex flex-col gap-4">
        {sessions.map((session, index) => (
          <div
            key={session.label}
            className={`rounded-2xl p-4 transition-all ${session.enabled ? "bg-white border border-gray-100 shadow-sm" : "bg-gray-50"}`}
          >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className={`
                  flex-1 bg-gray-50 border rounded-xl px-3 py-2 text-xs text-center font-medium outline-none
                  transition-all
                  ${session.enabled
                    ? "text-gray-700 border-gray-200 focus:border-green-500 cursor-pointer"
            ...
            {/* بيعرض الوقت بصيغة 12h تحت الـ inputs */}
            {session.enabled && (
