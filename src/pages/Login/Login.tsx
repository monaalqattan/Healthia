...
const EyeOpenIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="opacity-70">
    ...
    <div className="font-manrope flex min-h-screen w-full flex-col items-center overflow-x-hidden bg-[#EEF0EC]">
      ...
      <main className="relative z-10 flex w-full flex-1 items-center justify-center px-5 py-12">
        <div className="pointer-events-none absolute top-[10%] left-[5%] z-0 w-[250px] -rotate-[15deg] opacity-[0.08] blur-[1px] lg:w-[350px]">
          <img src={leafImg} alt="decorative leaf" className="h-full w-full object-contain" />
        </div>
        ...
        <div className="relative z-10 flex w-full max-w-[400px] flex-col rounded-[32px] bg-white px-10 py-12 shadow-[0_12px_40px_rgba(0,0,0,0.06)]">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className={`w-full rounded-xl border bg-[#F4F5F4] px-4 py-3.5 text-[14px] text-[#333] transition-all outline-none placeholder:text-[#bbb] ${
                  error.email ? "border-red-500 bg-red-50/10" : "border-[#EBEBEB] focus:border-[#1B7A4B] focus:bg-white"
                }`}
              ...
              />
              {error.email && (
                <p className="px-1 text-[11px] font-medium text-red-500 italic">{error.email}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative flex items-center">
                <input
                  ...
                  className={`w-full rounded-xl border bg-[#F4F5F4] px-4 py-3.5 pr-12 text-[14px] text-[#333] transition-all outline-none ${
                    error.password ? "border-red-500 bg-red-50/10" : "border-[#EBEBEB] focus:border-[#1B7A4B] focus:bg-white"
                  }`}
                  placeholder="••••••••"
                  value={password}
                  onChange={(e: ChangeEvent<HTMLInputElement>) => {
              ...
              {error.password && (
                <p className="px-1 text-[11px] font-medium text-red-500 italic">{error.password}</p>
              )}
            </div>
            ...
            <div className="mt-1 flex items-center gap-2.5">
              ...
              <span className="text-[13px] font-medium text-[#666]">Remember me</span>
            </div>

            {/* API Error */}
            {apiError && (
              <p className="text-center text-[12px] font-medium text-red-500">{apiError}</p>
            )}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-[#1B7A4B] py-4 text-[15px] font-bold text-white shadow-lg transition-all hover:bg-[#15613c] active:scale-[0.98] disabled:opacity-60"
            >
