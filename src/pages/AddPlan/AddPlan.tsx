...
import {
  PatientBar,
  DietCalculator,
} from "../../components/addplan/features/components"
...
import type {
  ...
  Equation,
  FoodItem,
} from "../../components/addplan/features/types"
import { DAYS_OF_WEEK } from "../../components/addplan/features/constants"
import {
  INITIAL_FORM,
  INITIAL_TARGETS,
} from "../../components/addplan/features/data"
...
export default function AddPlan() {
  ...
  const [weekMeals, setWeekMeals] =
    useState<Record<string, Meal[]>>(buildInitialWeek())
  ...
  const [savedPlans, setSavedPlans] = useState<PlanData[]>([])
    ...
    fetchPatientPlans(selectedPatientId)
      .then((plans: unknown[]) => {
        const plan = plans.find(
          (p: unknown) => (p as { _id: string })._id === editPlanId
        )
        if (!plan) return
  ...
  }, [editPlanId, selectedPatientId, handleFormChange])
  ...
  const totalEaten = currentMeals.reduce(
    (s, m) => s + m.foods.reduce((ss, f) => ss + f.kcal, 0),
    0
  )
    ...
    (plan: PlanData) => {
      ...
      if (plan.days && plan.days.length > 0) {
        ...
        plan.days.forEach((d) => {
          ...
          d.meals?.forEach((m) => {
            const key = typeToId[String(m?.type)] ?? "snack"
            mealMap[key].push({
              name: String(m?.name ?? ""),
  ...
  const handleSave = useCallback(async () => {
    ...
    const patientName =
      dbPatients.find((p) => p._id === selectedPatientId)?.name ?? "Patient"
  ...
  const handleExportPDF = () => {
    ...
    const getWeekDates = () => {
      ...
      const [y, m, d] = planStartDate
        ? planStartDate.split("-").map(Number)
        : [
      ...
      DAYS_OF_WEEK.forEach((day) => {
        ...
        result[day] = dt.toLocaleDateString("en-US", {
          weekday: "long",
          month: "long",
          day: "numeric",
          year: "numeric",
    ...
    const daysSections = DAYS_OF_WEEK.map((day) => {
      ...
      // حساب إجمالي اليوم
      const dayTotalCal = meals.reduce(
        (s, m) => s + m.foods.reduce((fs, f) => fs + (f.kcal || 0), 0),
        0
      )
      const dayTotalPro = meals.reduce(
        (s, m) => s + m.foods.reduce((fs, f) => fs + (f.protein || 0), 0),
        0
      )
      const dayTotalCarb = meals.reduce(
        (s, m) => s + m.foods.reduce((fs, f) => fs + (f.carbs || 0), 0),
        0
      )
      const dayTotalFat = meals.reduce(
        (s, m) => s + m.foods.reduce((fs, f) => fs + (f.fat || 0), 0),
        0
      )
    ...
    const startDateLabel = planStartDate
      ? new Date(
          ...(planStartDate.split("-").map(Number) as [number, number, number])
      ...
      <div className="addplan-page pt-6">
        ...
        <div className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="btn-outline" onClick={handleOpenPlans}>
              📂 Load Plan
            </button>
        ...
        <div className="content-grid">
          ...
          <div className="meal-section">
            ...
            <div className="cal-tracker">
              ...
              <div
                className={`cal-box ${remaining < 0 ? "bad" : remaining < 200 ? "warn" : "ok"}`}
              >
                <div className="cal-label">Remaining</div>
                <div className="cal-val">
                  {targets.cal ? remaining : "—"} kcal
                </div>
              </div>
            </div>
      ...
      {showPlansModal && (
        ...
        >
          ...
          >
            ...
            {/* Body */}
            <div
              style={{ overflowY: "auto", padding: "16px 24px 24px", flex: 1 }}
            >
              {loadingPlans ? (
                <p
                  style={{
                ...
                >
                  No saved plans for this patient
                </p>
              ) : (
                <div
                ...
                >
                  {savedPlans.map((plan) => {
                    const date = plan.startDate
                      ...
                      : "No date"
                    const daysWithFood = (plan.days ?? []).filter(
                      (d) => d.meals?.length > 0
                    ).length
                    return (
                      <button
